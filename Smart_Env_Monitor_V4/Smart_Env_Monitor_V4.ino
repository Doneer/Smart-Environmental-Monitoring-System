...
void loop() {
  //First check if we're in sleep mode
  checkSleepMode();
  
  //Skip the rest of the loop if in sleep mode
  if (isInSleepMode) {
    return;
  }
  ...
  if (autoRotate && millis() - lastModeChangeTime > 3000 && !inSettingsEdit) {
    menuPosition = (menuPosition + 1) % 3; // Reduced to 3 menu positions
...
void readSensors() {
  ...
  //Read light sensor (raw ADC value first)
...
void handleAlertSounds() {
  ...
  switch (currentAlertLevel) {
    case ALERT_HIGH:
      //Rapid toggling (200ms intervals)
    ...
    case ALERT_LOW:
      //Slower toggling (500ms on, 2000ms off)
      if (alertToggleState) {
...
int convertLightToPercent(int adcValue) {
  const int darkADC = 100;    //ADC value in very dim light
  const int brightADC = 900;  //ADC value in bright light
  
  //Constrain ADC reading to calibration range
  int constrained = constrain(adcValue, darkADC, brightADC);
  
  //Map to percentage with calibration points
